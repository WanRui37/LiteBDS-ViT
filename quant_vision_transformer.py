...
default_cfgs = {
    ...
    "vit_base_patch32_224": _cfg(
        url="", mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5)  # no official model weights for this combo, only for in21k
    ),
...
class Q_Mlp(nn.Module):
    ...
    def __init__(
        ...
        mlp_fc1_groups = (
            config.layer_groups["mlp_fc1"][layer_idx]
            if hasattr(config, "layer_groups") and config.layer_groups
            else config.ffn_group_num
        )
        mlp_fc2_groups = (
            config.layer_groups["mlp_fc2"][layer_idx]
            if hasattr(config, "layer_groups") and config.layer_groups
            else config.ffn_group_num
        )
        ...
        self.fc1 = LinearQ(
            ...
            linear_method=config.ffn_linear_method,
            shift_step=config.ffn_fc1_shift_step,
            num_groups=mlp_fc1_groups,
            learnable_groups=config.learnable_groups,
        )
        self.act = act_layer()
        ...
        self.fc2 = LinearQ(
            ...
            mode=Qmodes.kernel_wise,
            linear_method=config.ffn_linear_method,
            shift_step=config.ffn_fc2_shift_step,
            num_groups=mlp_fc2_groups,
            learnable_groups=config.learnable_groups,
        )
...
class Q_Attention(nn.Module):
    ...
    def __init__(
        ...
        # 获取当前层的分组数量
        attn_qkv_groups = (
            config.layer_groups["attn_qkv"][layer_idx]
            if hasattr(config, "layer_groups") and config.layer_groups
            else config.attn_group_num
        )
        attn_proj_groups = (
            config.layer_groups["attn_proj"][layer_idx]
            if hasattr(config, "layer_groups") and config.layer_groups
            else config.attn_group_num
        )
        ...
        if self.quantize_attn:
            ...
            self.qkv = LinearQ(
                ...
                nbits_w=nbits,
                mode=Qmodes.kernel_wise,
                linear_method=config.attn_linear_method,
                shift_step=config.attn_qkv_shift_step,
                num_groups=attn_qkv_groups,
                learnable_groups=config.learnable_groups,
            )
            ...
            self.proj = LinearQ(
                ...
                mode=Qmodes.kernel_wise,
                linear_method=config.attn_linear_method,
                shift_step=config.attn_proj_shift_step,
                num_groups=attn_proj_groups,
                learnable_groups=config.learnable_groups,
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs: additional arguments for compatibility with timm
        """
        super().__init__()
        ...
        self.head = (
            LinearQ(
                ...
                num_classes,
                nbits_w=8,
                linear_method=config.head_linear_method,
                shift_step=config.head_shift_step,
                num_groups=config.head_group_num,
            )
            if num_classes > 0
            else nn.Identity()
        )
...
@register_model
def eightbits_deit_small_patch16_224(config, pretrained=False, **kwargs):
...
@register_model
def eightbits_deit_small_patch16_224_div(config, pretrained=False, **kwargs):
    model = lowbit_VisionTransformer(
        config=config,
        nbits=8,
        patch_size=16,
...
@register_model
def fourbits_deit_small_patch16_224_h192(pretrained=False, **kwargs):
    model = lowbit_VisionTransformer(
        nbits=4,
        patch_size=16,
        embed_dim=192,
